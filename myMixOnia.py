...
def main():
    ...
    eventList = eu.MPSOniaMixer.mix_from_recipe(mixRecipe, maxEvents = 100000)
    eu.LHEParser("sample_pp_psipsi_sps.lhe").setEvents(eventList).write("psipsipsi_tps_mixed_raw.lhe")
    # Apply a filter to the events
    eventList_filtered = []
    for event in eventList:
        join = True
        for particle in event.particles:
            if particle.pdgid == 443 and abs(particle.p4.eta) > 2.5:
                join = False
                break
        if join:
            eventList_filtered.append(event)
    eu.LHEParser("sample_pp_psipsi_sps.lhe").setEvents(eventList_filtered).write("psipsipsi_tps_mixed.lhe")
